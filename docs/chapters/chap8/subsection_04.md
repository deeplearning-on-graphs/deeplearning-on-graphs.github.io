...
次元 $d$ でのノード $v_i$ の次元内近傍ノードの集合は $\symcal{N}\_d(v_i)$ と表記される．

図8.1で示した多次元グラフを例にこれらの近傍を考えてみよう． この図において，「B」次元にあるノード $4$ の次元内近傍はノード1，ノード2，ノード5である． 一方，全ての次元で同じノード $4$ が共有されており，これらは異なる次元での同じノードの「コピー」とみなすことができる． そして，ノード $4$ のこれらのコピーは暗黙的に互いに接続しており，これらのノードをノード $4$ の次元間近傍と呼ぶ． 図8.1で示されているように，「B」次元でのノード $4$ の次元間近傍ノードは，「A」と「C」の次元におけるノード $4$ 自身である． これら二つの近傍ノードを用いて，Ma *et al*.(2019c)で提案された多次元グラフ用の( $l$ 層目におけるノード $v_i$ に対する)グラフフィルタリング操作は，次のように説明される：
...
\begin{eqnarray}
    \symbf{F}^{(l-1)}_{d,j} &=& \sigma\left(\symbf{F}^{(l-1)}_j\symbf{\Theta}^{(l-1)}_d\right) \quad \text{for} \;\;v_j\in\symcal{N}_d(v_i)
\tag{8.1}\\
    \symbf{F}^{(l-1)}_{g,i} &=& \sigma\left(\symbf{F}^{(l-1)}_i\symbf{\Theta}^{(l-1)}_g\right)\quad \text{for} \;\; g = 1,\dots,D
\tag{8.2}\\
    \symbf{F}^{(l)}_{w,d,i} &=& \sum_{v_j\in\symcal{N}_d(v_i)}\symbf{F}^{(l-1)}_{d,j}
\tag{8.3}\\
