...
<figcaption>図12.1 典型的な交通予測フレームワークにおける学習層</figcaption>
...
空間型グラフフィルタリング操作の出力は系列 $(\symbf{F}^{(l)}\_{1,\symscr{S}},\dots,\symbf{F}^{(l)}\_{\symscr{M},\symscr{S}})$ としてまとめられ，これが以下のように時間的関連性を捉える系列モデルに供給される：
