...
&= \sum_{v\_j\in \symbfscr{N}(v\_i)}(\symbf{f}[i] - \symbf{f}[j]).\tag{2.9}\end{aligned} $$ 
式(2.9)から明らかのように， $\symbf{h}[i]$ は，ノード $v\_i$ とその近傍ノード $\symbfscr{N}(v\_i)$ との間における， $\symbf{f}$ の差を合計したものになる．次に， $\symbf{f}^{T}\symbf{L}\symbf{f}$ を以下のように求める．
 $$ \begin{aligned}
    ...
    &= \dfrac{1}{2}\sum_{v\_i\in \symbfscr{V}}\sum_{v\_j\in\symbfscr{V}(v\_i)}(\symbf{f}[i] - \symbf{f}[j])^{2}.\tag{2.10}\end{aligned} $$ 
