...
<strong>定義 2.45 グラフ分類</strong>
 ...
 $y_i$ をグラフ $\g_i$ のラベルとする，ラベル付きグラフの集合 $\symbfscr{D} = \left\{(\g_i, y_i)\right\}$ が与えられたとき，グラフ分類タスクの目的は， $\symbfscr{D}$ を使ってラベルのないグラフが持つラベルを予測するような写像 $\phi$ を学習することにある．
